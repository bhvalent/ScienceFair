		...
		if ($result && $result->num_rows > 0) {
			...
			if ($result && $result->num_rows > 0) {
				...
				for ($i = 0; $i < count($array); $i++) {
					$query5 = "Select distinct FKRegistrationID from TeamMembers ";
					$query5 .= "inner join Registration on FKRegistrationID = RegistrationID ";
					$query5 .= "inner join Class on TeamMembers.FKClassID = ClassID ";
					$query5 .= "where FKFairID = ".$ID." and Class = ".$array[$i];
			...
			echo "<p></p>";
			echo "<a href='exportProjectsClass.php?id=".$ID."' class='btn btn-success btn-block'>Export</a>";
			echo "</div>";
			echo "</div>";
